...
# ----------------------------- Load Dataset -----------------------------
df = pd.read_csv("../Dataset/Water_Quality_Dataset_Refined.csv")
...
# Save Random Forest
joblib.dump(rf_model, "../models/rf_water_model.pkl")

# Save XGBoost
joblib.dump(xgb_model, "../models/xgb_water_model.pkl")

X.columns

import pandas as pd
row_data = [
    "reservoir", 26.488, 29.002, 7.004, 0,
    2, 0.213, "Winter", 5.938, 5.125,
    5,4.77,8.251,353.79
]

# Create a DataFrame from the list, ensuring the column names match the training data
data_df = pd.DataFrame([row_data], columns=X.columns)
xgb_model.predict(data_df)

import joblib


# Load your model
model = joblib.load("../models/xgb_water_model.pkl")
